...
def main():
    ...
    with tab1:
        ...
        #US Map with bubbles and dark theme
        st.subheader("FDIC Fraud Cases Across the United States")
        try:
            # load us fraud data and cache
            @st.cache_data(show_spinner=False)
            def load_us_data(limit=5000):
                return supabase.table("final_article_label_dataset").select("location,fraud_group_primary").limit(limit).execute().data
            df = load_us_data()
            if not df:
                ...
                # extract city and state vectorized
                loc_split = df_map["location"].str.split(",", n=1, expand=True)
                df_map["city"] = loc_split[0].str.strip()
                df_map["state"] = loc_split[1].str.strip() if loc_split.shape[1]>1 else None
                # aggregate state counts
                state_counts = df_map.groupby("state").size().reset_index(name="cases")
                state_summary = df_map.groupby(["state","fraud_group_primary"]).size().reset_index(name="count")
                # generate hover texts
                hover_texts = {state:"<br>".join([f"{row['fraud_group_primary']}: {row['count']}" for _, row in state_summary[state_summary["state"]==state].sort_values("count",ascending=False).head(3).iterrows()]) for state in state_counts["state"].unique()}
                state_counts["hover_text"] = state_counts["state"].map(hover_texts)
                # create choropleth map
                fig = px.choropleth(state_counts,locations="state",locationmode="USA-states",color="cases",hover_name="state",hover_data={"cases":True,"hover_text":True},color_continuous_scale="Viridis",scope="usa",labels={"cases":"Fraud Cases"})
                fig.update_traces(hovertemplate="%{hover_name}<br>Cases: %{cases}<br>%{customdata[1]}")
                fig.update_layout(geo_bgcolor="black",paper_bgcolor="black",plot_bgcolor="black",margin={"r":0,"t":0,"l":0,"b":0},height=650)
                # aggregate city counts
                city_counts = df_map.groupby(["city","state"]).size().reset_index(name="count")
                city_counts = city_counts[city_counts["city"].notna()]
                city_counts["state_full"] = city_counts["state"].map(lambda x: us.states.lookup(x).name if us.states.lookup(x) else None)
                city_counts["location"] = city_counts["city"] + ", " + city_counts["state_full"].fillna("")
                # cache geocoding results
                @st.cache_data(show_spinner=False)
                def geocode_location(location):
                    try:
                        return geolocator.geocode(location) if location else None
                    except:
                        return None
                # geocode cities and filter valid coords
                geolocator = Nominatim(user_agent="fraud_dashboard")
                city_counts["coords"] = city_counts["location"].apply(geocode_location)
                city_counts["lat"] = city_counts["coords"].apply(lambda x: x.latitude if x else None)
                city_counts["lon"] = city_counts["coords"].apply(lambda x: x.longitude if x else None)
                city_counts = city_counts.dropna(subset=["lat","lon"])
                # add city bubbles to map
                if not city_counts.empty:
                    fig.add_scattergeo(lat=city_counts["lat"],lon=city_counts["lon"],text=city_counts["city"],marker=dict(size=city_counts["count"]*1.5,color="white",opacity=0.8),mode="markers")
    ...
    with tab3:
        ...
        st.write("This dashboard identifies rising fraud themes, shows trend charts, and provides cluster-level fraud group and detection insights.")
        if st.button("Analyze Emerging Signals"):
            with st.spinner("Analyzing"):
                # load data from supabase and cache
                @st.cache_data(show_spinner=False)
                def load_data(limit=5000):
                    article_rows = supabase.table("final_article_label_dataset") \
                                        .select("title,url,full_date,published,clean_text,summary,fraud_group_primary,fraud_group_secondary,detection_method,location,amount_involved,amount_numeric") \
                                        .limit(limit).execute().data
                    emb_rows = supabase.table("final_embeddings_dataset") \
                                    .select("url,embedding").limit(limit).execute().data
                ...
                if not article_rows or not emb_rows:
                    ...
                    for a in article_rows:
                        ...
                        if not url or emb is None:
                            continue
                        date_raw = a.get("full_date") or a.get("published") or ""
                        try:
                            parsed = pd.to_datetime(date_raw).strftime("%Y-%m-%d")
                        except Exception:
                            try:
                                parsed = pd.to_datetime(str(date_raw)[:4]).strftime("%Y-%m-%d")
                            except Exception:
                                parsed = ""
                        articles.append({
                            "title": a.get("title",""),
                    ...
                    if len(articles) == 0:
                        ...
                        # pca and clustering and cache
                        @st.cache_data(show_spinner=True)
                        def reduce_and_cluster(X, k=6):
                        ...
                        X_reduced, labels, kmeans = reduce_and_cluster(X, k=6)
                        df["cluster"] = labels
                        # show cluster charts
                        st.subheader("Cluster counts")
                        st.bar_chart(df["cluster"].value_counts().sort_index())
                        st.subheader("Cluster by fraud group")
                        fg_table = df.groupby(["cluster","fraud_group_primary"]).size().unstack(fill_value=0)
                        st.bar_chart(fg_table)
                        st.subheader("Cluster by detection method")
                        dm_table = df.groupby(["cluster","detection_method"]).size().unstack(fill_value=0)
                        st.bar_chart(dm_table)
                        # generate monthly trends
                        parsed_months = pd.to_datetime(df["date"].replace("", np.nan)).dt.to_period("M").astype(str).fillna("unknown")
                        df["ym"] = parsed_months
                        monthly = df[df["ym"]!="unknown"].groupby(["ym","cluster"]).size().unstack(fill_value=0).sort_index()
                        st.subheader("Fraud theme trends over time by cluster")
                        if not monthly.empty:
                            st.line_chart(monthly)
                        # generate augmented text
                        df["aug_text"] = (df["clean_text"].fillna("") + " " +
                                        df["fraud_group_primary"].fillna("") + " " +
                                        df["detection_method"].fillna("")).str.strip()
                        # compute cluster keywords and cache
                        @st.cache_data(show_spinner=True)
                        def compute_cluster_keywords(df, k):
                            cluster_candidates = {}
                            try:
                        ...
                        cluster_candidates = compute_cluster_keywords(df, k=6)
                        ...
                        for cl in range(6):
                            ...
                            reps = [{"title": df.loc[r]["title"], "url": df.loc[r]["url"], "date": df.loc[r]["date"], "summary": df.loc[r]["summary"] or df.loc[r]["clean_text"][:400]} for r in rep_idxs]
                        ...
                        if monthly_sorted.shape[0] >= 2:
                            for col in monthly_sorted.columns:
                                ...
                                if len(series) < recent_window+1:
                                    growth_scores[col] = slope_scores[col] = novelty_scores[col] = 0.0
                                    continue
                                ...
                                growth_scores[col] = float((recent.mean() - past_mean) / past_std) if past_std>0 else 0.0
                                slope_scores[col] = float(np.polyfit(np.arange(len(recent)), recent.values, 1)[0])
                                prev_sum = past.sum() if len(past)>0 else 0.0
                                novelty_scores[col] = float((recent.sum()+1)/(prev_sum+1))
                        else:
                            for col in range(6):
                                growth_scores[col] = slope_scores[col] = 0.0
                        ...
                        g_norm, s_norm, n_norm = normalize_dict(growth_scores), normalize_dict(slope_scores), normalize_dict(novelty_scores)
                        emerg_scores = {col: 0.5*g_norm.get(col,0.0)+0.3*s_norm.get(col,0.0)+0.2*n_norm.get(col,0.0) for col in growth_scores.keys()}
